...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [proposal] = await getDb().insert(proposals).values({ opportunityId, number, validUntil: payload.validUntil ? String(payload.validUntil) : null, discount, subtotal, total: subtotal - discount, notes: String(payload.notes ?? "").trim() || null }).returning();
