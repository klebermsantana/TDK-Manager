...
const values = (payload: Record<string, unknown>) => ({
  ...
  competitivePrice: Number(payload.competitivePrice ?? 0),
  standardPrice: Number(payload.standardPrice ?? 0),
  valuePrice: Number(payload.valuePrice ?? 0),
});
const valid = (item: ReturnType<typeof values>) =>
  categories.has(item.category) &&
  item.description &&
  [item.cost, item.competitivePrice, item.standardPrice, item.valuePrice].every(
...
export async function POST(request: Request) {
  const denied = await requirePermission("proposals");
