...
export async function GET() {
  ...
  try {
    ...
    const totalCapacityFactor = routingProfiles.reduce((sum, row) => sum + availabilityFactor(row.availability), 0);
    ...
    const people: PersonLoad[] = users.map((user) => {
      ...
      const factor = availabilityFactor(routing.availability);
    ...
    for (const item of unassigned) {
      ...
      suggestions.push({
        ...
        reason: `${treasurySkillLabels[target.skill]} em ${item.domain === "cash" ? "Caixa" : item.domain === "critical_tasks" ? "Pendências críticas" : item.domain === "reconciliation" ? "Conciliação" : "Fechamento"}; ${target.person.availability === "limited" ? "disponibilidade limitada" : "disponível"} e menor carga compatível entre os candidatos elegíveis.`,
      });
      ...
      for (const source of people.filter((person) => person.state === "overloaded" || person.availability === "unavailable")) {
        ...
        for (const item of owned) {
          ...
          suggestions.push({
            ...
            targetSkillLevel: target.skill,
            targetAvailability: target.person.availability,
            reason: source.availability === "unavailable"
              ? `${source.name} está indisponível. ${target.person.name} é ${treasurySkillLabels[target.skill].toLowerCase()} na competência exigida e está elegível para receber o item.`
              : `Alivia uma pessoa sobrecarregada e direciona para ${treasurySkillLabels[target.skill].toLowerCase()} na competência exigida, priorizando disponibilidade e menor carga normalizada.`,
          });
    ...
    return Response.json({
      ...
      methodology: {
        ...
        note: "Sugestões consideram disponibilidade e competência antes da carga. Disponível é priorizado sobre Limitado; nível 0 e Indisponível ficam fora do roteamento assistido. A carga ponderada continua sendo heurística operacional, não nota de desempenho.",
      },
    });
  } catch (error) {
