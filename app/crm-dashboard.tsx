...
function Projects({sales,update}:{sales:SaleRecord[];update:(sale:SaleRecord,changes:Partial<SaleRecord>)=>void}){
 ...
 return <><div className="project-flow-note"><ClipboardList/><div><strong>Acompanhamento operacional</strong><span>Cada pedido gera um projeto independente. Alterações aqui não mudam o status comercial do pedido nem impedem o faturamento.</span></div></div><div className="project-summary"><span><strong>{sales.filter(item=>!["concluido","cancelado"].includes(item.projectStatus)).length}</strong> projetos ativos</span><span><strong>{sales.filter(item=>item.projectStatus==="andamento").length}</strong> em andamento</span><span><strong>{sales.filter(item=>item.scheduledEnd&&item.scheduledEnd<new Date().toISOString().slice(0,10)&&!["concluido","cancelado"].includes(item.projectStatus)).length}</strong> atrasados</span></div><div className="filter-bar project-filters"><Field label="Pedido, cliente ou projeto"><Input value={filters.search} onChange={event=>setFilters({...filters,search:event.target.value})} placeholder="Buscar projeto..."/></Field><Field label="Responsável"><Input value={filters.manager} onChange={event=>setFilters({...filters,manager:event.target.value})} placeholder="Nome do responsável"/></Field><Field label="Status do projeto"><select value={filters.status} onChange={event=>setFilters({...filters,status:event.target.value})}><option value="ativos">Projetos ativos</option><option value="todos">Todos</option><option value="aguardando">Aguardando</option><option value="andamento">Em andamento</option><option value="concluido">Concluído</option><option value="cancelado">Cancelado</option></select></Field></div>{visible.length?<div className="project-grid">{visible.map(item=><ProjectCard key={`${item.id}-${item.updatedAt??""}`} item={item} update={update}/>)}</div>:<div className="filter-empty">Nenhum projeto encontrado com esses filtros.</div>}</>;
}
