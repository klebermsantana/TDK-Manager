    ...
    async function load() {
      try {
        const responses = await Promise.all(
          ["/api/opportunities", "/api/companies", "/api/contacts", "/api/activities", "/api/proposals", "/api/catalog", "/api/sales", "/api/billings", "/api/receivables", "/api/payables", "/api/team", "/api/settings", "/api/goals", "/api/project-tasks"].map((url) =>
      ...
      <main className="main">
        ...
        <div className="content">
          ...
          {loading ? (
            ...
            <Projects sales={sales} tasks={projectTasks} team={team.filter(member=>member.active)} update={updateSale} addTask={addProjectTask} toggleTask={toggleProjectTask} deleteTask={deleteProjectTask} saving={saving} />
          ) : view === "sales" ? (
...
function Projects({sales,tasks,team,update,addTask,toggleTask,deleteTask,saving}:{sales:SaleRecord[];tasks:ProjectTask[];team:TeamMember[];update:(sale:SaleRecord,changes:Partial<SaleRecord>)=>void;addTask:(saleId:number,task:{title:string;responsible:string;dueDate:string})=>void;toggleTask:(task:ProjectTask)=>void;deleteTask:(task:ProjectTask)=>void;saving:boolean}){
