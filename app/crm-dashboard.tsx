...
type ServiceCall={id:number;number:string;companyId:number|null;companyName:string;serviceTaker:string|null;requestOrigin:string|null;department:string|null;customerTicket:string|null;saleId:number|null;location:string|null;contactName:string|null;technician:string|null;serviceType:string;priority:string;scheduledAt:string|null;status:string;subject:string;description:string;executedService:string|null;consumablesUsed:boolean;consumablesDescription:string|null;partsReplaced:boolean;partsDescription:string|null;expensesAmount:number;expensesDescription:string|null;createdBy:string;createdAt:string;updatedAt:string};
...
type ServiceCallFile={id:number;serviceCallId:number;name:string;contentType:string;size:number;uploadedBy:string;createdAt:string};
...
export default function CrmDashboard({
  ...
  const [selectedServiceCallId,setSelectedServiceCallId]=useState<number|null>(null);
  ...
  const [serviceCallForm,setServiceCallForm]=useState({companyId:"",serviceTaker:"",requestOrigin:"telefone",department:"",customerTicket:"",saleId:"",location:"",contactName:"",technician:"",serviceType:"visita",priority:"normal",scheduledAt:"",subject:"",description:"",executedService:"",consumablesUsed:false,consumablesDescription:"",partsReplaced:false,partsDescription:"",expensesAmount:"0",expensesDescription:""});
    ...
    async function load() {
      try {
        const responses = await Promise.all(
          ["/api/opportunities", "/api/companies", "/api/contacts", "/api/activities", "/api/proposals", "/api/catalog", "/api/sales", "/api/billings", "/api/receivables", "/api/payables", "/api/team", "/api/settings", "/api/goals", "/api/project-tasks", "/api/project-files", "/api/service-calls", "/api/service-call-files"].map((url) =>
        ...
        if (active) {
          ...
          setServiceCallFiles(payloads[16].files);
        }
      } catch (reason) {
        if (active)
          setError(
  ...
  function openNew(kind: "company" | "contact" | "opportunity" | "activity" | "goal" | "proposal" | "catalog" | "payable" | "member" | "service_call") {
    ...
    if (kind === "service_call") setServiceCallForm({companyId:"",serviceTaker:"",requestOrigin:"telefone",department:"",customerTicket:"",saleId:"",location:"",contactName:"",technician:"",serviceType:"visita",priority:"normal",scheduledAt:"",subject:"",description:"",executedService:"",consumablesUsed:false,consumablesDescription:"",partsReplaced:false,partsDescription:"",expensesAmount:"0",expensesDescription:""});
    setDialog(kind);
  }
  ...
  function editProposal(proposal: ProposalRecord) {
    ...
    setProposalForm({ opportunityId:proposal.opportunityId?String(proposal.opportunityId):"",companyId:proposal.companyId?String(proposal.companyId):"",customerOrder:proposal.customerOrder??"",requester:proposal.requester??"",priceTable:proposal.priceTable??"padrao",validUntil:proposal.validUntil??"",discount:String(proposal.discount),notes:proposal.notes??"",items:proposal.items.map(item=>({catalogId:item.catalogId,category:item.category,description:item.description,quantity:item.quantity,unitCost:item.unitCost??0,unitPrice:item.unitPrice})) });
  ...
  async function updateSale(sale:SaleRecord,changes:Partial<SaleRecord>){const next={...sale,...changes};setSales(current=>current.map(item=>item.id===sale.id?next:item));try{const response=await fetch("/api/sales",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(next)});const data=await response.json();if(!response.ok)throw new Error(data.error);setSales(current=>current.map(item=>item.id===sale.id?{...next,...data.sale,items:next.items}:item));}catch(reason){setSales(current=>current.map(item=>item.id===sale.id?sale:item));setError(reason instanceof Error?reason.message:"Não foi possível atualizar o pedido ou projeto.");}}
  async function addProjectTask(saleId:number,task:{title:string;responsible:string;dueDate:string}){setSaving(true);setError("");try{const response=await fetch("/api/project-tasks",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({saleId,...task})});const data=await response.json();if(!response.ok)throw new Error(data.error);setProjectTasks(current=>[...current,data.task]);setSales(current=>current.map(sale=>sale.id===saleId?{...sale,progress:data.progress,projectStatus:data.progress>0?"andamento":"aguardando"}:sale));}catch(reason){setError(reason instanceof Error?reason.message:"Não foi possível cadastrar a tarefa.");}finally{setSaving(false);}}
  async function toggleProjectTask(task:ProjectTask){setError("");try{const response=await fetch("/api/project-tasks",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:task.id,completed:!task.completedAt})});const data=await response.json();if(!response.ok)throw new Error(data.error);setProjectTasks(current=>current.map(item=>item.id===task.id?data.task:item));setSales(current=>current.map(sale=>sale.id===task.saleId?{...sale,progress:data.progress,projectStatus:data.progress===100?"concluido":data.progress>0?"andamento":"aguardando"}:sale));}catch(reason){setError(reason instanceof Error?reason.message:"Não foi possível atualizar a tarefa.");}}
  async function deleteProjectTask(task:ProjectTask){if(!window.confirm(`Excluir a tarefa “${task.title}”?`))return;setError("");try{const response=await fetch(`/api/project-tasks?id=${task.id}`,{method:"DELETE"});const data=await response.json();if(!response.ok)throw new Error(data.error);setProjectTasks(current=>current.filter(item=>item.id!==task.id));setSales(current=>current.map(sale=>sale.id===task.saleId?{...sale,progress:data.progress,projectStatus:data.progress===100?"concluido":data.progress>0?"andamento":"aguardando"}:sale));}catch(reason){setError(reason instanceof Error?reason.message:"Não foi possível excluir a tarefa.");}}
  async function uploadProjectFile(saleId:number,file:File){setSaving(true);setError("");try{const body=new FormData();body.append("saleId",String(saleId));body.append("file",file);const response=await fetch("/api/project-files",{method:"POST",body});const data=await response.json();if(!response.ok)throw new Error(data.error);setProjectFiles(current=>[...current,data.file]);}catch(reason){setError(reason instanceof Error?reason.message:"Não foi possível enviar o arquivo.");}finally{setSaving(false);}}
  async function deleteProjectFile(file:ProjectFile){if(!window.confirm(`Excluir o arquivo “${file.name}”?`))return;setError("");try{const response=await fetch(`/api/project-files?id=${file.id}`,{method:"DELETE"});const data=await response.json();if(!response.ok)throw new Error(data.error);setProjectFiles(current=>current.filter(item=>item.id!==file.id));}catch(reason){setError(reason instanceof Error?reason.message:"Não foi possível excluir o arquivo.");}}
  ...
  function editServiceCall(call:ServiceCall){setEditingId(call.id);setServiceCallForm({companyId:String(call.companyId??""),serviceTaker:call.serviceTaker??"",requestOrigin:call.requestOrigin??"telefone",department:call.department??"",customerTicket:call.customerTicket??"",saleId:String(call.saleId??""),location:call.location??"",contactName:call.contactName??"",technician:call.technician??"",serviceType:call.serviceType,priority:call.priority,scheduledAt:call.scheduledAt?.slice(0,16)??"",subject:call.subject,description:call.description,executedService:call.executedService??"",consumablesUsed:call.consumablesUsed,consumablesDescription:call.consumablesDescription??"",partsReplaced:call.partsReplaced,partsDescription:call.partsDescription??"",expensesAmount:String(call.expensesAmount??0),expensesDescription:call.expensesDescription??""});setSelectedServiceCallId(null);setDialog("service_call");}
  async function createServiceCall(event:React.FormEvent){event.preventDefault();const company=companies.find(item=>item.id===Number(serviceCallForm.companyId));if(!company)return;const current=serviceCalls.find(item=>item.id===editingId);setSaving(true);setError("");try{const response=await fetch("/api/service-calls",{method:editingId?"PATCH":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...serviceCallForm,id:editingId,status:current?.status??"triagem",companyName:company.name})});const data=await response.json();if(!response.ok)throw new Error(data.error);if(editingId)setServiceCalls(items=>items.map(item=>item.id===editingId?data.call:item));else{setServiceCalls(items=>[data.call,...items]);setServiceCallHistory(items=>[...items,{id:Date.now(),serviceCallId:data.call.id,fromStatus:null,toStatus:"triagem",changedBy:user.name,createdAt:data.call.createdAt}]);}setDialog(null);setEditingId(null);}catch(reason){setError(reason instanceof Error?reason.message:"Não foi possível salvar o chamado.");}finally{setSaving(false);}}
  ...
  async function uploadServiceCallPhoto(serviceCallId:number,file:File){setSaving(true);setError("");try{const body=new FormData();body.append("serviceCallId",String(serviceCallId));body.append("file",file);const response=await fetch("/api/service-call-files",{method:"POST",body});const data=await response.json();if(!response.ok)throw new Error(data.error);setServiceCallFiles(current=>[...current,data.file]);}catch(reason){setError(reason instanceof Error?reason.message:"Não foi possível enviar a foto.");}finally{setSaving(false);}}
  async function deleteServiceCallPhoto(file:ServiceCallFile){if(!window.confirm(`Excluir a foto “${file.name}”?`))return;try{const response=await fetch(`/api/service-call-files?id=${file.id}`,{method:"DELETE"});const data=await response.json();if(!response.ok)throw new Error(data.error);setServiceCallFiles(current=>current.filter(item=>item.id!==file.id));}catch(reason){setError(reason instanceof Error?reason.message:"Não foi possível excluir a foto.");}}
  ...
  const selectedServiceCall=serviceCalls.find(call=>call.id===selectedServiceCallId)??null;
  ...
  const alerts: { id: string; title: string; detail: string; view: View; tone: "danger" | "warning" }[] = [
    ...
    ...(can("sales") ? sales
      .filter(sale => sale.scheduledEnd && sale.scheduledEnd < todayKey && !["concluido", "cancelado"].includes(sale.projectStatus))
    ...
    ...(can("sales") ? projectTasks
      .filter(task => !task.completedAt && task.dueDate && task.dueDate < todayKey)
      .map(task => { const sale=sales.find(item=>item.id===task.saleId);return { id: `project-task-${task.id}`, title: "Tarefa de projeto atrasada", detail: `${task.title} • ${sale?.companyName??"Projeto"}`, view: "projects" as View, tone: "danger" as const }; }) : []),
  ];
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main">
        ...
        <div className="content">
          ...
          {loading ? (
            ...
            <ServiceCalls calls={serviceCalls} history={serviceCallHistory} changeStatus={changeServiceCallStatus} inspect={call=>setSelectedServiceCallId(call.id)} edit={editServiceCall} />
          ) : view === "sales" ? (
            ...
            <Proposals proposals={proposals} add={() => openNew("proposal")} inspect={(proposal) => setSelectedProposalId(proposal.id)} />
          ) : view === "activities" ? (
      ...
      <Sheet open={Boolean(selectedServiceCall)} onOpenChange={open=>{if(!open)setSelectedServiceCallId(null)}}>
        <SheetContent className="opportunity-sheet service-call-sheet">
          {selectedServiceCall&&<><SheetHeader className="opportunity-sheet-head"><span className="dialog-kicker">ORDEM DE SERVIÇO</span><SheetTitle>{selectedServiceCall.number}</SheetTitle><SheetDescription>{selectedServiceCall.companyName} · {selectedServiceCall.subject}</SheetDescription></SheetHeader><div className="opportunity-sheet-body"><div className={`service-call-detail-priority priority-${selectedServiceCall.priority}`}><span>Prioridade</span><strong>{selectedServiceCall.priority}</strong></div><div className="service-call-detail-grid"><div><span>Status</span><select value={selectedServiceCall.status} onChange={event=>changeServiceCallStatus(selectedServiceCall,event.target.value)}>{serviceCallStatuses.map(status=><option key={status.id} value={status.id}>{status.label}</option>)}</select></div><div><span>Técnico responsável</span><strong>{selectedServiceCall.technician||"Não definido"}</strong></div><div><span>Agendamento</span><strong>{selectedServiceCall.scheduledAt?new Intl.DateTimeFormat("pt-BR",{dateStyle:"short",timeStyle:"short"}).format(new Date(selectedServiceCall.scheduledAt)):"Não agendado"}</strong></div><div><span>Tipo de serviço</span><strong>{selectedServiceCall.serviceType}</strong></div><div><span>Solicitante</span><strong>{selectedServiceCall.contactName||"Não informado"}</strong></div><div><span>Local</span><strong>{selectedServiceCall.location||"Não informado"}</strong></div></div><section className="service-call-description"><h3>Descrição do chamado</h3><p>{selectedServiceCall.description}</p></section><section className="service-call-quick-actions"><h3>Avançar atendimento</h3><div>{serviceCallStatuses.filter(status=>!["triagem","cancelado"].includes(status.id)).map(status=><Button key={status.id} variant={selectedServiceCall.status===status.id?"default":"outline"} onClick={()=>changeServiceCallStatus(selectedServiceCall,status.id)}>{status.label}</Button>)}</div></section><section className="opportunity-history"><div className="history-head"><h3>Histórico do chamado</h3><span>{serviceCallHistory.filter(item=>item.serviceCallId===selectedServiceCall.id).length}</span></div>{serviceCallHistory.filter(item=>item.serviceCallId===selectedServiceCall.id).slice().reverse().map(item=><article key={item.id}><i/><div><strong>{item.fromStatus?`${serviceCallStatuses.find(status=>status.id===item.fromStatus)?.label??item.fromStatus} → `:"Chamado aberto em "}{serviceCallStatuses.find(status=>status.id===item.toStatus)?.label??item.toStatus}</strong><span>{new Intl.DateTimeFormat("pt-BR",{dateStyle:"short",timeStyle:"short"}).format(new Date(item.createdAt))}</span></div><small>{item.changedBy}</small></article>)}</section></div></>}
        </SheetContent>
      </Sheet>
      ...
      <Dialog open={dialog === "service_call"} onOpenChange={(open)=>setDialog(open?"service_call":null)}>
        <DialogContent className="dialog service-call-dialog">
          <DialogHeader><span className="dialog-kicker">ORDEM DE SERVIÇO</span><DialogTitle>{editingId?"Editar chamado":"Novo chamado"}</DialogTitle><DialogDescription>{editingId?"Complete as informações necessárias para avançar o atendimento.":"Registre inicialmente a ocorrência para triagem."}</DialogDescription></DialogHeader>
          <form onSubmit={createServiceCall} className="form">
            ...
            <div className="form-split"><Field label="Origem da solicitação"><select value={serviceCallForm.requestOrigin} onChange={event=>setServiceCallForm({...serviceCallForm,requestOrigin:event.target.value})}><option value="email">E-mail</option><option value="whatsapp">WhatsApp</option><option value="telefone">Ligação</option><option value="presencial">Presencial</option><option value="outro">Outro</option></select></Field><Field label="Tomador do serviço"><Input value={serviceCallForm.serviceTaker} onChange={event=>setServiceCallForm({...serviceCallForm,serviceTaker:event.target.value})} placeholder="Empresa ou unidade contratante"/></Field></div>
            ...
            <div className="form-split"><Field label="Departamento"><Input value={serviceCallForm.department} onChange={event=>setServiceCallForm({...serviceCallForm,department:event.target.value})} placeholder="Setor que necessita do atendimento"/></Field><Field label="Chamado interno do cliente"><Input value={serviceCallForm.customerTicket} onChange={event=>setServiceCallForm({...serviceCallForm,customerTicket:event.target.value})} placeholder="Número de controle do cliente"/></Field></div>
            ...
            {editingId&&<section className="service-execution-form"><header><Wrench/><div><strong>Execução técnica</strong><small>Preencha durante ou após o atendimento.</small></div></header><Field label="Serviço executado"><Textarea value={serviceCallForm.executedService} onChange={event=>setServiceCallForm({...serviceCallForm,executedService:event.target.value})} placeholder="Descreva detalhadamente o diagnóstico e o serviço realizado"/></Field><label className="service-execution-check"><input type="checkbox" checked={serviceCallForm.consumablesUsed} onChange={event=>setServiceCallForm({...serviceCallForm,consumablesUsed:event.target.checked})}/><span>Houve utilização de materiais de consumo</span></label>{serviceCallForm.consumablesUsed&&<Field label="Materiais de consumo"><Textarea value={serviceCallForm.consumablesDescription} onChange={event=>setServiceCallForm({...serviceCallForm,consumablesDescription:event.target.value})} placeholder="Informe item, quantidade e unidade"/></Field>}<label className="service-execution-check"><input type="checkbox" checked={serviceCallForm.partsReplaced} onChange={event=>setServiceCallForm({...serviceCallForm,partsReplaced:event.target.checked})}/><span>Houve troca de equipamentos, partes ou peças</span></label>{serviceCallForm.partsReplaced&&<Field label="Equipamentos, partes e peças"><Textarea value={serviceCallForm.partsDescription} onChange={event=>setServiceCallForm({...serviceCallForm,partsDescription:event.target.value})} placeholder="Informe equipamento, marca, modelo, patrimônio ou serial e quantidade"/></Field>}<div className="form-split"><Field label="Despesas do atendimento (R$)"><Input type="number" min="0" step="0.01" value={serviceCallForm.expensesAmount} onChange={event=>setServiceCallForm({...serviceCallForm,expensesAmount:event.target.value})}/></Field><Field label="Descrição das despesas"><Input value={serviceCallForm.expensesDescription} onChange={event=>setServiceCallForm({...serviceCallForm,expensesDescription:event.target.value})} placeholder="Pedágio, estacionamento, frete..."/></Field></div><div className="service-evidence"><div><strong>Fotos e evidências</strong><small>{serviceCallFiles.filter(file=>file.serviceCallId===editingId).length} foto(s) anexada(s)</small></div><label className={saving?"disabled":""}><Upload/>Adicionar fotos<input type="file" accept="image/*" capture="environment" multiple disabled={saving} onChange={event=>{Array.from(event.target.files??[]).forEach(file=>uploadServiceCallPhoto(editingId,file));event.currentTarget.value="";}}/></label></div>{serviceCallFiles.some(file=>file.serviceCallId===editingId)&&<div className="service-evidence-grid">{serviceCallFiles.filter(file=>file.serviceCallId===editingId).map(file=><figure key={file.id}><a href={`/api/service-call-files?id=${file.id}`} target="_blank" rel="noreferrer"><img src={`/api/service-call-files?id=${file.id}`} alt={file.name}/></a><figcaption><span>{file.name}</span><button type="button" onClick={()=>deleteServiceCallPhoto(file)} aria-label={`Excluir ${file.name}`}><Trash2/></button></figcaption></figure>)}</div>}</section>}
            <small className="service-call-rule-note">Para mudar o status para Aberto, preencha tomador, local, contato, departamento, chamado interno, modalidade e data e hora do agendamento.</small><SaveButton saving={saving} disabled={!companies.length}>{editingId?"Salvar informações":"Cadastrar em triagem"}</SaveButton>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={dialog === "proposal"} onOpenChange={(open) => { setDialog(open ? "proposal" : null); if (!open) setEditingProposalId(null); }}>
        <DialogContent className="dialog proposal-dialog">
          ...
          <form onSubmit={createProposal} className="form proposal-form">
            <div className="proposal-top"><Field label="Oportunidade (opcional)"><select value={proposalForm.opportunityId} onChange={(event) => { const opportunity=items.find(item=>item.id===Number(event.target.value)); const company=companies.find(entry=>entry.id===opportunity?.companyId); const priceTable=company?.preferredPriceTable??proposalForm.priceTable; setProposalForm(current=>({...current,opportunityId:event.target.value,companyId:opportunity?.companyId?String(opportunity.companyId):current.companyId,priceTable,items:current.items.map(entry=>{const catalogItem=catalog.find(item=>item.id===entry.catalogId);return catalogItem?{...entry,unitPrice:catalogPrice(catalogItem,priceTable)}:entry;})})); }}><option value="">Proposta direta</option>{items.map(item=><option key={item.id} value={item.id}>{item.title} — {item.company}</option>)}</select></Field><Field label="Empresa"><select value={proposalForm.companyId} onChange={(event)=>{const company=companies.find(item=>item.id===Number(event.target.value));const priceTable=company?.preferredPriceTable??proposalForm.priceTable;setProposalForm(current=>({...current,companyId:event.target.value,opportunityId:"",priceTable,items:current.items.map(entry=>{const catalogItem=catalog.find(item=>item.id===entry.catalogId);return catalogItem?{...entry,unitPrice:catalogPrice(catalogItem,priceTable)}:entry;})}));}} required={!proposalForm.opportunityId}><option value="">Selecione</option>{companies.map(company=><option key={company.id} value={company.id}>{company.name}</option>)}</select></Field><Field label="Tabela de preços"><select value={proposalForm.priceTable} onChange={(event)=>{const priceTable=event.target.value;setProposalForm(current=>({...current,priceTable,items:current.items.map(entry=>{const catalogItem=catalog.find(item=>item.id===entry.catalogId);return catalogItem?{...entry,unitPrice:catalogPrice(catalogItem,priceTable)}:entry;})}));}}><option value="competitiva">Competitiva</option><option value="padrao">Padrão</option><option value="valor">Valor agregado</option></select></Field><Field label="Validade"><Input type="date" value={proposalForm.validUntil} onChange={event=>setProposalForm({...proposalForm,validUntil:event.target.value})}/></Field></div>
            <div className="proposal-reference"><Field label="Pedido/OC do cliente"><Input value={proposalForm.customerOrder} onChange={event=>setProposalForm({...proposalForm,customerOrder:event.target.value})} placeholder="Número ou referência"/></Field><Field label="Solicitante / responsável"><Input value={proposalForm.requester} onChange={event=>setProposalForm({...proposalForm,requester:event.target.value})} placeholder="Nome do solicitante"/></Field></div>
            ...
            {catalog.length ? <Field label="Buscar no catálogo"><CatalogPicker catalog={catalog} priceTable={proposalForm.priceTable} onSelect={addCatalogItem}/></Field>:null}
...
function Receivables({receivables,update}:{receivables:ReceivableRecord[];update:(item:ReceivableRecord,changes:Partial<ReceivableRecord>)=>void}){
 ...
 return <div className="receivable-page"><div className="receivable-metrics"><span>A receber<strong>{money(open)}</strong></span><span>Vencido<strong>{money(due)}</strong></span><span>Recebido<strong>{money(received)}</strong></span></div><div className="receivable-list">{receivables.map(item=><ReceivableRow key={`${item.id}-${item.updatedAt}`} item={item} today={today} update={update}/>)}</div></div>;
}
function ReceivableRow({item,today,update}:{item:ReceivableRecord;today:string;update:(item:ReceivableRecord,changes:Partial<ReceivableRecord>)=>void}){
 const [draft,setDraft]=useState(item);const overdue=item.status!=="recebido"&&item.dueDate<today;const dirty=["interest","penalty","discount","receivedAmount","paymentDate"].some(key=>draft[key as keyof ReceivableRecord]!==item[key as keyof ReceivableRecord]);
 const reverse=()=>{if(window.confirm("Estornar este recebimento e reabrir a parcela?"))update(item,{receivedAmount:0,paymentDate:null});};
 return <article className={`receivable-row ${overdue?"overdue":""}`}><div className="receivable-title"><small>{item.billingNumber} · Parcela {item.installmentNumber}</small><strong>{item.companyName}</strong><span>Vence em {new Intl.DateTimeFormat("pt-BR",{timeZone:"UTC"}).format(new Date(`${item.dueDate}T12:00:00Z`))}</span></div><span className="receivable-amount">{money(item.amount)}</span><div className="receivable-inputs"><Field label="Juros"><Input type="number" min="0" step="0.01" value={draft.interest} onChange={e=>setDraft({...draft,interest:Number(e.target.value)})}/></Field><Field label="Multa"><Input type="number" min="0" step="0.01" value={draft.penalty} onChange={e=>setDraft({...draft,penalty:Number(e.target.value)})}/></Field><Field label="Desconto"><Input type="number" min="0" step="0.01" value={draft.discount} onChange={e=>setDraft({...draft,discount:Number(e.target.value)})}/></Field><Field label="Recebido"><Input type="number" min="0" step="0.01" value={draft.receivedAmount} onChange={e=>setDraft({...draft,receivedAmount:Number(e.target.value)})}/></Field><Field label="Pagamento"><Input type="date" value={draft.paymentDate??""} onChange={e=>setDraft({...draft,paymentDate:e.target.value||null})}/></Field></div><b className={`receivable-badge ${item.status}`}>{item.status==="recebido"?"Recebida":item.status==="parcial"?"Parcial":overdue?"Vencida":"Em aberto"}</b><div className="receivable-actions">{(item.receivedAmount>0||item.paymentDate)&&<Button variant="outline" className="reverse-receivable" disabled={dirty} onClick={reverse}>Estornar recebimento</Button>}<Button disabled={!dirty} onClick={()=>update(item,draft)}>Salvar alterações</Button><Button variant="outline" disabled={!dirty} onClick={()=>setDraft(item)}>Cancelar edição</Button></div></article>;
}
function Billings({billings,update,generate}:{billings:BillingRecord[];update:(billing:BillingRecord,changes:Partial<BillingRecord>)=>void;generate:(billing:BillingRecord)=>void}){
 const [client,setClient]=useState("");const [order,setOrder]=useState("");const [status,setStatus]=useState("todos");const [from,setFrom]=useState("");const [to,setTo]=useState("");
 ...
 const filtered=billings.filter(item=>{const date=item.createdAt.slice(0,10);return(!client||normalized(item.companyName).includes(normalized(client)))&&(!order||normalized(`${item.number} ${item.saleNumber}`).includes(normalized(order)))&&(status==="todos"||item.status===status)&&(!from||date>=from)&&(!to||date<=to)});
 return <><div className="filter-bar billing-filters"><Field label="Cliente"><Input placeholder="Buscar cliente" value={client} onChange={event=>setClient(event.target.value)}/></Field><Field label="Pedido / faturamento"><Input placeholder="Ex.: PED-2026" value={order} onChange={event=>setOrder(event.target.value)}/></Field><Field label="Status"><select value={status} onChange={event=>setStatus(event.target.value)}><option value="todos">Todos</option><option value="pendente">Pendente</option><option value="parcial">Parcial</option><option value="recebido">Recebido</option></select></Field><Field label="De"><Input type="date" value={from} onChange={event=>setFrom(event.target.value)}/></Field><Field label="Até"><Input type="date" value={to} onChange={event=>setTo(event.target.value)}/></Field></div>{filtered.length?<div className="billing-list">{filtered.map(item=><BillingCard key={`${item.id}-${item.updatedAt}`} item={item} update={update} generate={generate}/>)}</div>:<div className="filter-empty">Nenhum faturamento encontrado com esses filtros.</div>}</>;
}
function BillingCard({item,update,generate}:{item:BillingRecord;update:(billing:BillingRecord,changes:Partial<BillingRecord>)=>void;generate:(billing:BillingRecord)=>void}){
...
function ServiceCalls({calls,history,changeStatus,inspect,edit}:{calls:ServiceCall[];history:ServiceCallHistory[];changeStatus:(call:ServiceCall,status:string)=>void;inspect:(call:ServiceCall)=>void;edit:(call:ServiceCall)=>void}){
 ...
 const card=(call:ServiceCall)=><article className={`service-call-card priority-${call.priority}`} key={call.id} draggable onDragStart={()=>setDragged(call.id)} onDragEnd={()=>setDragged(null)} onClick={()=>inspect(call)}><header><span>{call.number}</span><b>{call.priority}</b></header><h3>{call.subject}</h3><p>{call.companyName}</p>{call.location&&<small>Local: {call.location}</small>}<div className="service-call-meta"><span>{call.technician||"Sem técnico"}</span><span>{call.scheduledAt?new Intl.DateTimeFormat("pt-BR",{dateStyle:"short",timeStyle:"short"}).format(new Date(call.scheduledAt)):"Sem agendamento"}</span></div><footer><select aria-label={`Status de ${call.number}`} value={call.status} onClick={event=>event.stopPropagation()} onChange={event=>changeStatus(call,event.target.value)}>{serviceCallStatuses.map(status=><option key={status.id} value={status.id}>{status.label}</option>)}</select><button type="button" className="service-card-edit" onClick={event=>{event.stopPropagation();edit(call)}}><Pencil/> Editar</button><small>{lastChange(call.id)?`Alterado por ${lastChange(call.id)?.changedBy}`:`Aberto por ${call.createdBy}`}</small></footer></article>;
...
function Projects({sales,tasks,files,team,update,addTask,toggleTask,deleteTask,uploadFile,deleteFile,saving}:{sales:SaleRecord[];tasks:ProjectTask[];files:ProjectFile[];team:TeamMember[];update:(sale:SaleRecord,changes:Partial<SaleRecord>)=>void;addTask:(saleId:number,task:{title:string;responsible:string;dueDate:string})=>void;toggleTask:(task:ProjectTask)=>void;deleteTask:(task:ProjectTask)=>void;uploadFile:(saleId:number,file:File)=>void;deleteFile:(file:ProjectFile)=>void;saving:boolean}){
 ...
 const normalized=(value:string)=>value.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();
 const visible=sales.filter(item=>(!filters.search||normalized(`${item.number} ${item.companyName} ${item.opportunityTitle}`).includes(normalized(filters.search)))&&(filters.status==="todos"||(filters.status==="ativos"?!["concluido","cancelado"].includes(item.projectStatus):item.projectStatus===filters.status))&&(!filters.manager||normalized(`${item.projectManager??""} ${item.projectMembers.map(id=>team.find(member=>String(member.id)===id)?.name??"").join(" ")}`).includes(normalized(filters.manager))));
 ...
 return <><div className="project-flow-note"><ClipboardList/><div><strong>Acompanhamento operacional</strong><span>Cada pedido gera um projeto independente. As tarefas concluídas calculam automaticamente o progresso da execução.</span></div></div><div className="project-summary"><span><strong>{sales.filter(item=>!["concluido","cancelado"].includes(item.projectStatus)).length}</strong> projetos ativos</span><span><strong>{sales.filter(item=>item.projectStatus==="andamento").length}</strong> em andamento</span><span><strong>{tasks.filter(task=>!task.completedAt&&task.dueDate&&task.dueDate<new Date().toISOString().slice(0,10)).length}</strong> tarefas atrasadas</span></div><div className="filter-bar project-filters"><Field label="Pedido, cliente ou projeto"><Input value={filters.search} onChange={event=>setFilters({...filters,search:event.target.value})} placeholder="Buscar projeto..."/></Field><Field label="Responsável"><Input value={filters.manager} onChange={event=>setFilters({...filters,manager:event.target.value})} placeholder="Nome do responsável"/></Field><Field label="Status do projeto"><select value={filters.status} onChange={event=>setFilters({...filters,status:event.target.value})}><option value="ativos">Projetos ativos</option><option value="todos">Todos</option><option value="aguardando">Aguardando</option><option value="andamento">Em andamento</option><option value="concluido">Concluído</option><option value="cancelado">Cancelado</option></select></Field></div>{visible.length?<div className="project-grid">{visible.map(item=><ProjectCard key={`${item.id}-${item.updatedAt??""}-${item.progress}-${item.projectStatus}`} item={item} tasks={tasks.filter(task=>task.saleId===item.id)} files={files.filter(file=>file.saleId===item.id)} team={team} update={update} addTask={addTask} toggleTask={toggleTask} deleteTask={deleteTask} uploadFile={uploadFile} deleteFile={deleteFile} saving={saving}/>)}</div>:<div className="filter-empty">Nenhum projeto encontrado com esses filtros.</div>}</>;
}
function ProjectCard({item,tasks,files,team,update,addTask,toggleTask,deleteTask,uploadFile,deleteFile,saving}:{item:SaleRecord;tasks:ProjectTask[];files:ProjectFile[];team:TeamMember[];update:(sale:SaleRecord,changes:Partial<SaleRecord>)=>void;addTask:(saleId:number,task:{title:string;responsible:string;dueDate:string})=>void;toggleTask:(task:ProjectTask)=>void;deleteTask:(task:ProjectTask)=>void;uploadFile:(saleId:number,file:File)=>void;deleteFile:(file:ProjectFile)=>void;saving:boolean}){
 const [taskForm,setTaskForm]=useState({title:"",responsible:"",dueDate:""});
 const [draft,setDraft]=useState(item);const today=new Date().toISOString().slice(0,10);const overdue=Boolean(item.scheduledEnd&&item.scheduledEnd<today&&!["concluido","cancelado"].includes(item.projectStatus));const fields:(keyof SaleRecord)[]=["projectStatus","scheduledStart","scheduledEnd","projectManager","projectMembers","progress","projectNotes"];const dirty=fields.some(key=>draft[key]!==item[key]);
 const save=()=>update(item,{projectStatus:draft.projectStatus,scheduledStart:draft.scheduledStart,scheduledEnd:draft.scheduledEnd,projectManager:draft.projectManager,projectMembers:draft.projectMembers,progress:Number(draft.progress),projectNotes:draft.projectNotes});
 const submitTask=(event:React.FormEvent)=>{event.preventDefault();if(!taskForm.title.trim())return;addTask(item.id,taskForm);setTaskForm({title:"",responsible:"",dueDate:""});};
 const allocated=team.filter(member=>draft.projectMembers.includes(String(member.id)));
 return <article className={`project-card ${overdue?"overdue":""}`}><header><span><ClipboardList/></span><div><small>{item.number} · Pedido {item.status==="concluido"?"concluído":item.status==="andamento"?"em andamento":item.status==="cancelado"?"cancelado":"aguardando"}</small><h2>{item.companyName}</h2><p>{item.opportunityTitle}</p></div><b className={`project-status ${draft.projectStatus}`}>{overdue?"Atrasado":draft.projectStatus==="andamento"?"Em andamento":draft.projectStatus==="concluido"?"Concluído":draft.projectStatus==="cancelado"?"Cancelado":"Aguardando"}</b></header><div className={`project-progress ${tasks.length?"calculated":""}`}><div><span>Progresso do projeto {tasks.length?<small>calculado pelas tarefas</small>:null}</span><strong>{Number(draft.progress)||0}%</strong></div><i><b style={{width:`${Math.min(100,Math.max(0,Number(draft.progress)||0))}%`}}/></i>{!tasks.length&&<input aria-label="Progresso do projeto" type="range" min="0" max="100" step="5" value={Number(draft.progress)||0} onChange={event=>setDraft({...draft,progress:Number(event.target.value),projectStatus:Number(event.target.value)>0&&draft.projectStatus==="aguardando"?"andamento":draft.projectStatus})}/>}</div><div className="project-fields"><Field label="Status do projeto"><select value={draft.projectStatus} onChange={event=>setDraft({...draft,projectStatus:event.target.value,progress:event.target.value==="concluido"?100:draft.progress})}><option value="aguardando">Aguardando</option><option value="andamento">Em andamento</option><option value="concluido">Concluído</option><option value="cancelado">Cancelado</option></select></Field><Field label="Gestor do projeto"><select value={draft.projectManager??""} onChange={event=>setDraft({...draft,projectManager:event.target.value||null})}><option value="">Selecione</option>{team.map(member=><option key={member.id} value={member.name}>{member.name}</option>)}</select></Field><Field label="Início"><Input type="date" value={draft.scheduledStart??""} onChange={event=>setDraft({...draft,scheduledStart:event.target.value||null})}/></Field><Field label="Conclusão prevista"><Input type="date" value={draft.scheduledEnd??""} onChange={event=>setDraft({...draft,scheduledEnd:event.target.value||null})}/></Field></div><div className="project-team"><span>Equipe técnica</span><div>{team.map(member=><label key={member.id}><input type="checkbox" checked={draft.projectMembers.includes(String(member.id))} onChange={event=>setDraft({...draft,projectMembers:event.target.checked?[...draft.projectMembers,String(member.id)]:draft.projectMembers.filter(id=>id!==String(member.id))})}/><i>{initials(member.name)}</i><b>{member.name}</b></label>)}</div></div><Field label="Observações da execução"><Textarea value={draft.projectNotes??""} onChange={event=>setDraft({...draft,projectNotes:event.target.value})} placeholder="Pendências, alinhamentos e informações importantes do projeto"/></Field><section className="project-tasks"><div className="project-tasks-head"><strong>Etapas e tarefas</strong><span>{tasks.filter(task=>task.completedAt).length}/{tasks.length} concluídas</span></div>{tasks.length?<div className="project-task-list">{tasks.map(task=>{const taskOverdue=Boolean(!task.completedAt&&task.dueDate&&task.dueDate<today);return <div className={`project-task ${task.completedAt?"done":""} ${taskOverdue?"late":""}`} key={task.id}><button type="button" onClick={()=>toggleTask(task)} aria-label={task.completedAt?"Reabrir tarefa":"Concluir tarefa"}><CheckCircle2/></button><div><strong>{task.title}</strong><small>{task.responsible||"Sem responsável"}{task.dueDate?` · ${new Intl.DateTimeFormat("pt-BR",{timeZone:"UTC"}).format(new Date(`${task.dueDate}T12:00:00Z`))}`:" · Sem prazo"}{taskOverdue?" · Atrasada":""}</small></div><button type="button" className="project-task-delete" onClick={()=>deleteTask(task)} aria-label="Excluir tarefa"><Trash2/></button></div>})}</div>:<p className="project-tasks-empty">Adicione a primeira tarefa para calcular o progresso automaticamente.</p>}<form className="project-task-form" onSubmit={submitTask}><Input value={taskForm.title} onChange={event=>setTaskForm({...taskForm,title:event.target.value})} placeholder="Nova tarefa ou etapa" required/><select aria-label="Responsável pela tarefa" value={taskForm.responsible} onChange={event=>setTaskForm({...taskForm,responsible:event.target.value})}><option value="">Responsável</option>{allocated.map(member=><option key={member.id} value={member.name}>{member.name}</option>)}</select><Input type="date" aria-label="Prazo da tarefa" value={taskForm.dueDate} onChange={event=>setTaskForm({...taskForm,dueDate:event.target.value})}/><Button disabled={saving}><Plus/> Adicionar</Button></form></section><section className="project-files"><div className="project-files-head"><div><Paperclip/><span><strong>Documentos e evidências</strong><small>{files.length} arquivo{files.length===1?"":"s"}</small></span></div><label className={saving?"disabled":""}><Upload/> Anexar arquivo<input type="file" disabled={saving} accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.png,.jpg,.jpeg,.webp" onChange={event=>{const file=event.target.files?.[0];if(file)uploadFile(item.id,file);event.currentTarget.value="";}}/></label></div>{files.length?<div className="project-file-list">{files.map(file=><div className="project-file" key={file.id}><Paperclip/><div><strong>{file.name}</strong><small>{(file.size/1024/1024).toFixed(file.size>1048576?1:2)} MB · {file.uploadedBy}</small></div><a href={`/api/project-files?id=${file.id}`} target="_blank" rel="noreferrer" aria-label={`Baixar ${file.name}`}><Download/></a><button type="button" onClick={()=>deleteFile(file)} aria-label={`Excluir ${file.name}`}><Trash2/></button></div>)}</div>:<p>Nenhum documento anexado a este projeto.</p>}</section><footer><span>{money(item.total)} · {item.items.length} itens</span><div><Button variant="outline" disabled={!dirty} onClick={()=>setDraft(item)}>Cancelar alterações</Button><Button disabled={!dirty} onClick={save}>Salvar projeto</Button></div></footer></article>;
}
function Sales({sales,proposals,billings,update,bill}:{sales:SaleRecord[];proposals:ProposalRecord[];billings:BillingRecord[];update:(sale:SaleRecord,changes:Partial<SaleRecord>)=>void;bill:(sale:SaleRecord)=>void}){
  const [filters,setFilters]=useState({number:"",client:"",status:"",from:"",to:""});
  ...
  const normalized=(value:string)=>value.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();
  const visible=sales.filter(sale=>{const created=sale.createdAt.slice(0,10),proposal=proposals.find(item=>item.id===sale.proposalId);return(!filters.number||normalized(`${sale.number} ${proposal?.number??""}`).includes(normalized(filters.number)))&&(!filters.client||normalized(`${sale.companyName} ${sale.opportunityTitle}`).includes(normalized(filters.client)))&&(!filters.status||sale.status===filters.status)&&(!filters.from||created>=filters.from)&&(!filters.to||created<=filters.to)});
  return <><div className="filter-bar sales-filters"><Field label="Número do pedido / proposta"><Input placeholder="Ex.: PED-2026" value={filters.number} onChange={event=>setFilters({...filters,number:event.target.value})}/></Field><Field label="Cliente / projeto"><Input placeholder="Buscar cliente ou projeto" value={filters.client} onChange={event=>setFilters({...filters,client:event.target.value})}/></Field><Field label="Status"><select value={filters.status} onChange={event=>setFilters({...filters,status:event.target.value})}><option value="">Todos</option>{Object.entries(labels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></Field><Field label="Data inicial"><Input type="date" value={filters.from} onChange={event=>setFilters({...filters,from:event.target.value})}/></Field><Field label="Data final"><Input type="date" value={filters.to} onChange={event=>setFilters({...filters,to:event.target.value})}/></Field></div>{visible.length?<div className="sales-grid">{visible.map(sale=>{const margin=sale.total?(sale.total-sale.cost)/sale.total*100:0;const proposal=proposals.find(item=>item.id===sale.proposalId);return <article className="sale-card" key={sale.id}><div className="sale-head"><span><ShoppingCart/></span><div><small>{sale.number} · {proposal?.number}</small><h2>{sale.companyName}</h2><p>{sale.opportunityTitle}</p></div><select value={sale.status} onChange={event=>update(sale,{status:event.target.value})}>{Object.entries(labels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div><div className="sale-values"><span>Venda<strong>{money(sale.total)}</strong></span><span>Custo<strong>{money(sale.cost)}</strong></span><span>Margem<strong>{margin.toFixed(1)}%</strong></span></div><div className="sale-schedule"><Field label="Início previsto"><Input type="date" value={sale.scheduledStart??""} onChange={event=>update(sale,{scheduledStart:event.target.value||null})}/></Field><Field label="Conclusão prevista"><Input type="date" value={sale.scheduledEnd??""} onChange={event=>update(sale,{scheduledEnd:event.target.value||null})}/></Field></div>{sale.status==="concluido"&&!billings.some(item=>item.saleId===sale.id)&&<Button className="billing-button" onClick={()=>bill(sale)}><ReceiptText/> Preparar faturamento</Button>}<footer><span>{sale.items.length} itens</span><strong>{labels[sale.status]}</strong></footer></article>;})}</div>:<div className="filter-empty">Nenhum pedido encontrado com esses filtros.</div>}</>;
}
...
function CatalogPicker({catalog,priceTable,onSelect}:{catalog:CatalogItem[];priceTable:string;onSelect:(id:string)=>void}){
 const [search,setSearch]=useState("");const [open,setOpen]=useState(false);const results=catalog.filter(item=>`${item.code??""} ${item.description} ${item.category}`.toLowerCase().includes(search.toLowerCase())).slice(0,20);const price=(item:CatalogItem)=>priceTable==="competitiva"?item.competitivePrice:priceTable==="valor"?item.valuePrice:item.standardPrice;
 return <div className="catalog-picker"><Search/><input value={search} onFocus={()=>setOpen(true)} onChange={event=>{setSearch(event.target.value);setOpen(true)}} onBlur={()=>setTimeout(()=>setOpen(false),150)} placeholder="Digite código, produto ou serviço..." aria-label="Buscar no catálogo" autoComplete="off"/>{open&&<div className="catalog-picker-results" role="listbox">{results.length?results.map(item=><button type="button" role="option" aria-selected="false" key={item.id} onMouseDown={event=>event.preventDefault()} onClick={()=>{onSelect(String(item.id));setSearch("");setOpen(false)}}><span><small>{item.category==="material"?"Material":"Serviço"}{item.code?` · ${item.code}`:""}</small><strong>{item.description}</strong></span><b>{money(price(item))}</b></button>):<p>Nenhum item encontrado.</p>}</div>}</div>;
}
function Proposals({ proposals, add, inspect }: { proposals: ProposalRecord[]; add: () => void; inspect: (proposal: ProposalRecord) => void }) {
  ...
  const [filters,setFilters]=useState({client:"",opportunity:"",status:"",from:"",to:""});
  if (!proposals.length) return <Empty icon={<FileText />} title="Nenhuma proposta cadastrada" text="Crie uma proposta direta para um cliente ou vinculada a uma oportunidade." action="Criar proposta" onClick={add} />;
  const filtered=proposals.filter(item=>(!filters.client||item.companyName.toLowerCase().includes(filters.client.toLowerCase()))&&(!filters.opportunity||item.opportunityTitle.toLowerCase().includes(filters.opportunity.toLowerCase()))&&(!filters.status||item.status===filters.status)&&(!filters.from||item.createdAt.slice(0,10)>=filters.from)&&(!filters.to||item.createdAt.slice(0,10)<=filters.to));
  return <><div className="filter-bar proposal-filters"><Input placeholder="Cliente" value={filters.client} onChange={e=>setFilters({...filters,client:e.target.value})}/><Input placeholder="Oportunidade" value={filters.opportunity} onChange={e=>setFilters({...filters,opportunity:e.target.value})}/><select value={filters.status} onChange={e=>setFilters({...filters,status:e.target.value})}><option value="">Todos os status</option>{Object.entries(labels).map(([value,label])=><option value={value} key={value}>{label}</option>)}</select><Input type="date" aria-label="Data inicial" value={filters.from} onChange={e=>setFilters({...filters,from:e.target.value})}/><Input type="date" aria-label="Data final" value={filters.to} onChange={e=>setFilters({...filters,to:e.target.value})}/></div>{filtered.length?<div className="proposal-grid">{filtered.map(proposal=><article className="proposal-card" key={proposal.id} onClick={()=>inspect(proposal)}><div className="proposal-card-head"><span><FileText/></span><div><small>{proposal.number}</small><h2>{proposal.companyName}</h2></div><b className={`proposal-status ${proposal.status}`}>{labels[proposal.status]}</b></div><p>{proposal.opportunityTitle}</p><div className="proposal-breakdown"><span>Materiais <strong>{money(proposal.items.filter(item=>item.category==="material").reduce((sum,item)=>sum+Number(item.total??0),0))}</strong></span><span>Serviços <strong>{money(proposal.items.filter(item=>item.category==="servico").reduce((sum,item)=>sum+Number(item.total??0),0))}</strong></span></div><div className="proposal-card-total"><span>{proposal.customerOrder?`Pedido ${proposal.customerOrder}`:labels[proposal.status]}</span><div><small>{proposal.discount?`Desconto: ${money(proposal.discount)}`:"Sem desconto"}</small><strong>{money(proposal.total)}</strong></div></div></article>)}</div>:<div className="filter-empty">Nenhuma proposta encontrada com esses filtros.</div>}</>;
}
