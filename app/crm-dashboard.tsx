      ...
      <main className="main">
        ...
        <div className="content">
          ...
          {loading ? (
            ...
            <Projects sales={sales} tasks={projectTasks} update={updateSale} addTask={addProjectTask} toggleTask={toggleProjectTask} deleteTask={deleteProjectTask} saving={saving} />
          ) : view === "sales" ? (
...
function Projects({sales,tasks,update,addTask,toggleTask,deleteTask,saving}:{sales:SaleRecord[];tasks:ProjectTask[];update:(sale:SaleRecord,changes:Partial<SaleRecord>)=>void;addTask:(saleId:number,task:{title:string;responsible:string;dueDate:string})=>void;toggleTask:(task:ProjectTask)=>void;deleteTask:(task:ProjectTask)=>void;saving:boolean}){
 const [filters,setFilters]=useState({search:"",status:"ativos",manager:""});
 const normalized=(value:string)=>value.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();
 const visible=sales.filter(item=>(!filters.search||normalized(`${item.number} ${item.companyName} ${item.opportunityTitle}`).includes(normalized(filters.search)))&&(filters.status==="todos"||(filters.status==="ativos"?!["concluido","cancelado"].includes(item.projectStatus):item.projectStatus===filters.status))&&(!filters.manager||normalized(item.projectManager??"").includes(normalized(filters.manager))));
